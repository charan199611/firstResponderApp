...
@Component({
  selector: 'app-iot-incident-details',
  templateUrl: './iot-incident-details.page.html',
  styleUrls: ['./iot-incident-details.page.scss'],
})
export class IotIncidentDetailsPage implements OnInit {
  ...
  static instance: IotIncidentDetailsPage;
  imieNum = "";
 language="";
 livestreamButton = false;
 liveStreamUrl;
  ...
  constructor(private theDB: SqliteDb, private eventService: EventsDbService, private http: CustomHttpClient, private ngZone: NgZone,
    ...
    IotIncidentDetailsPage.instance = this;
    ...
    this.pagesubscription = this.messageService.getMessage().subscribe(message => {
      ...
      if (getMessgae == "hotUpdate") {
        ...
        this.ngZone.run(() => {
          ...
          //  //  IotIncidentDetailsPage.instance.setEventData();
  ...
  getFetchAttachedDetails(){
    ...
    this.http.post('Responder/fetch_attached_files', data).subscribe(data => {
     ...
     let mediaList = data.value;
      console.log("medialist", mediaList);

      for(var i=0; i<mediaList.length; i++ ){
        if(mediaList.file_name == "iveStreamUrl"){
          this.livestreamButton = true;
          this.liveStreamUrl = mediaList.file_path;
        }
      }
  ...
  setLiveStreamButton(){
    if(this.eventDetails.livestreamUrl == "" || this.eventDetails.livestreamUrl == "undefined" || this.eventDetails.livestreamUrl == undefined){
this.livestreamButton = false;
    }else{
      this.livestreamButton=true;
    }
  }
  ...
  loadLiveStram(selectedCFS){
    let data = { eventDetails: this.liveStreamUrl }
  ...
  backFunction() {
  this.customNavService.navigateRoot('/iot-incident/iot-incident-list');	
  }
  ...
  updateMissionStatusConfirmToDB(basicId, eventid, statusId, latitude, longitude, fromUserType, considerAtSceneLoc, isSubmitforClose, cityId, cityZoneId, deptId, psId, fromUserName, priority) {
    ...
    this.http.post('Responder/update_event_status', data).subscribe(data => {
      ...
      this.eventService.updateIotEventStatus(statusId, statusName, eventid);
      ...
      if (statusId === 9 || statusId == '9') {
        ...
        this.eventService.deleteIotEvent(basicId).then((value) => {
  ...
  loadATRPage(selectedCFS) {
    let data = { eventDetails: selectedCFS ,servicedata: "iotevent"}
      ...
      let alertBox = await this.alertCtrl.create({
        ...
        buttons: [ {
          ...
          handler: (data) => {
            console.log('Confirm Okay', data);
            this.ngZone.run(() => {
              IotIncidentDetailsPage.instance.selectedCFS.subtype = data.event_name;
              IotIncidentDetailsPage.instance.selectedCFS.eventtype = eventType;
              IotIncidentDetailsPage.instance.selectedCFS.subeventTypeId = data.event_id
              let eventmainId = event_id;
              console.log("this.selectedCFS.subtype", this.selectedCFS.subtype);


             
              this.updateEventTypeSubTypeDetails(IotIncidentDetailsPage.instance.selectedCFS.subeventTypeId,eventmainId, IotIncidentDetailsPage.instance.selectedCFS.subtype, IotIncidentDetailsPage.instance.selectedCFS.eventtype);
...
let params;
 ...
 this.httpClient.post('Responder/updateEventFromMDT', params).subscribe(result => {
    ...
    if(result.status == true){
      ...
      // this.eventService.updateEventTypeSubType(IotIncidentDetailsPage.instance.selectedCFS.eventtype, IotIncidentDetailsPage.instance.selectedCFS.subtype, IotIncidentDetailsPage.instance.selectedCFS.subeventTypeId, this.selectedCFS.eventid)
      // this.eventService.updateUnitSummaryEventTypeSubType(IotIncidentDetailsPage.instance.selectedCFS.eventtype, IotIncidentDetailsPage.instance.selectedCFS.subtype, this.selectedCFS.eventid)
