...
@Component({
  selector: 'app-iot-incident-list',
  templateUrl: './iot-incident-list.page.html',
  styleUrls: ['./iot-incident-list.page.scss'],
})
export class IotIncidentListPage implements OnInit {
  ...
  loadCFSDetails(cfsId, unreadStatus, basicId, fromUserType) {
    ...
    this.customNavService.navigateByUrl('/iot-incident/iot-incident-details', data)
  ...
  fetchTodayCFS() {
    ...
    this.subscription = this.eventService.getIotEventsAll().subscribe(message => {
  ...
  fetch7DaysCFSData() {
    ...
    this.eventService.getWeekCfsIotList(moment().subtract(7, 'days').startOf('day').format('YYYY-MM-DD HH:mm:ss'), moment().endOf('day').format('YYYY-MM-DD HH:mm:ss')).then((message: any[]) => {
      this.rowData = message;
      this.cfsList = [];
      this.showStatus(this.filterStatus);
      // if (this.rowData.length > 0) {
  ...
  fetch30DaysCFSData() {
    this.eventService.getWeekCfsIotList(moment().subtract(30, 'days').startOf('day').format('YYYY-MM-DD HH:mm:ss').toString(), moment().endOf('day').format('YYYY-MM-DD HH:mm:ss').toString()).then((message: any[]) => {
      this.rowData = message;
      this.cfsList = [];
      this.showStatus(this.filterStatus);
      //  if (this.rowData.length > 0) {
  ...
  async filterCfsList() {
    ...
    this.translateService.get(['Please Choose any Status', 'Cancel', 'Ok', 'All', 'Acknowledge', 'On Action', 'At Scene', 'Close', 'IOT EVENT RECEIVED']).subscribe(text => {
      ...
      this.missionStatus[5].status_level = text['IOT EVENT RECEIVED'];
    });
